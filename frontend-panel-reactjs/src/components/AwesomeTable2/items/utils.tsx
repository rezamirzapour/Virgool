...
export const getSortIcon = (sorts: ISort[], column: IColumn, handleSort: (column: IColumn, value: ISortValue) => void, classes: any): ReactChild => {
    const value = sorts.find((s) => s.field === column.field)?.value
    switch (value) {
        case null: return <ArrowLeftIcon
            style={{ transform: 'translateY(10px)' }}
            onClick={() => handleSort(column, 'asc')}
            className={classes.sortIcon}
        />
        case 'asc': return <ArrowDropDownIcon
            onClick={() => handleSort(column, 'desc')}
            className={classes.sortIcon}
        />
        case 'desc': return <ArrowDropUpIcon
