...
import { Grid, Select, ListItem, Button, Chip } from '@material-ui/core';
...
export default function AwesomeFilter(props: IProps) {
    const { onApplyFilter, register: { filterOptions, onFilterChange, getValues, onDeleteFilter } } = props;
        ...
        switch (currentFilter?.type) {
            case 'STRING': return (
                <TextFieldFilter
                    ...
                    onChange={ev => onTextFieldFilterChange(currentFilter.label, ev.target.value)}
                />
            )
            case 'SELECT': return (
                <SelectFilter
                    ...
                    selectProps={{
                        value: currentFilter?.value,
                        onChange: ev => onTextFieldFilterChange(currentFilter.label, ev.target.value as string)
                    }}
                />
            )
            case 'DATE_TIME_RANGE': return (
                <DateTimeRangeFilter
                    value={currentFilter.value}
                    onChange={value => onDateTimeRangeChange(currentFilter.label, value)}
                />
            )
            case 'NUMBER_RANGE': return (
                <NumberRangeFilter
                    value={currentFilter.value}
                    onChange={value => onNumberRangeChange(currentFilter.label, value)}
                />
            )
        }
    }
    ...
    return (
        <Grid container spacing={3} alignItems="center">
            ...
            <Grid item lg={3}>
                ...
                }} endIcon={<SortIcon />}>اعمال فیلتر</Button>
            </Grid>
            <Grid item xs={12}>
                {Object.entries(appliedFilters).map(([label, value]) => <Chip
                    key={label}
                    label={`${label}: ${value}`}
                    color="primary"
