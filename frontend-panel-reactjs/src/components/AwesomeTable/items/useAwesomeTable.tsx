...
const paginationReducer = (
  state: IPagination = initialPagination,
  action: IAction
): IPagination => {
  switch (action.type) {
    case "RESET":
    ...
    default:
      return state || initialPagination;
  }
};
...
export function useAwesomeTable<ResponseType>() {
  ...
  const setPage = (page: number): void =>
    void dispatchPagination({ type: "SET_PAGE", payload: page });
  const setSize = (size: number): void =>
    void dispatchPagination({ type: "SET_SIZE", payload: size });
  const fetchData = async (
    fetcherFn: () => Promise<AxiosResponse<ResponseType>>
  ): Promise<void> => {
    setLoading(true);
    try {
