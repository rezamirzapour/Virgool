...
import { CategoriesServices, CategoriesResponse } from 'services/categories';
...
export default function CategoriesList() {
    const { fetchData, loading, pagination, response, setPage, setSize } = useAwesomeTable<CategoriesResponse>();
    const { getValues, register } = useAwesomeFilter(initialFilterOptions());
    const { navigate } = useRouter();
    const { mutate, isSubmitting } = useMutate()

    const onApplyFiler = () => fetchData(() => CategoriesServices.findAll({ offset: pagination.offset, size: pagination.size, ...getValues() }))
            ...
            actions={[
                {
                    ...
                    onClick: (rd) => navigate('categories.edit', { params: { id: rd.id } }),
                    tooltip: 'ویرایش',
                },
                {
