...
export class PermissionsService {
  ...
  async findAll(listPermissoinsParams: ListPermissoinsParams) {
    try {
      ...
      if (paginate) {
        const result = await this.permissionRepository.findAndCountAll({ ...paginateOptions, ...options, attributes: ['id', 'title'] })
        return new PaginateResponse(result.rows, result.count);
      }
      const result = await this.permissionRepository.findAll({ ...options, attributes: ['id', 'title'] })
  ...
  async update(id: number, updatePermissionDto: UpdatePermissionDto) {
    try {
      await this.permissionRepository.update(updatePermissionDto, { where: { id } })
