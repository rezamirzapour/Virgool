...
import { CreateResponse, DestroyResponse, FindAllResponse, FindOneResponse, PaginateResponse } from 'common/httpResponse';
...
export class CommentsService {
  ...
  async create(createCommentDto: CreateCommentDto, author: User) {
    try {
      const result = await this.commentRepository.create({ ...createCommentDto, authorId: author.id });
  ...
  async findAll(listCommentsParams: ListCommentsParams) {
    ...
    if (paginate) {
      const result = await this.commentRepository.findAndCountAll({ ...paginateOptions, ...options })
